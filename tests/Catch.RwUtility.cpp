...
TEST_CASE("utility::invoke_blockable_async_callback - `nullptr` in `object_` argument results in error", "[utility::invoke_blockable_async_callback]") {
    // This requirement is confirmed by the compiler do to the templated reference type.
}

TEST_CASE("utility::invoke_blockable_async_callback - An `object_` argument not of templated type results in error", "[utility::invoke_blockable_async_callback]") {
    // This requirement is confirmed by the compiler do to the templated reference type.
}
...
TEST_CASE("utility::invoke_blockable_async_callback - The function shall return the result of the underlying function", "[utility::invoke_blockable_async_callback]") {
    TestClass test;
    test.testMethod_result = ENOTTY;
    REQUIRE(ENOTTY == utility::invoke_blockable_async_callback(test, &TestClass::testMethod, nullptr, nullptr));
}

TEST_CASE("utility::invoke_blockable_async_callback - The function shall return an error if the underlying function fails to complete", "[utility::invoke_blockable_async_callback]") {
    errno = 0;
    TestClass test;
    test.testMethod_should_hang = true;
    REQUIRE(ETIME == utility::invoke_blockable_async_callback(test, &TestClass::testMethod, nullptr, nullptr));
}
